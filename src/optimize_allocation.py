"""Reproduce representative heat-resilient allocation scenarios."""
...
from analysis_config import OUTPUTS, spatial_units_path
...
def main() -> None:
    ...
    rows = []
    for scenario in STRATEGIES.values():
        strategy = assign_strategy(df, scenario)
        result = evaluate(df, strategy)
        result["scenario"] = scenario
        result["changed_units"] = int((strategy != 0).sum())
        rows.append(result)

    out = root / args.output if args.output else OUTPUTS / "allocation_scenarios.csv"
